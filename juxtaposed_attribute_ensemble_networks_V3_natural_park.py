...
###########################
# NOTE: Previously download and save locally the forestfires.csv dataseet from Kaggle at
...
# Binarize target variable depending on if it is less or equal than 10 
# or not
data[target_variable] = data[target_variable].apply(lambda x: 0 if x == 0 else 1)
...
for target_value in sorted(list(data[target_variable].unique())):
    print("\nCreating attribute models for cases when target is " + str(target_value) + "...\n##########################")
...
recall = recall_score(target, cv_predictions)
f1 = f1_score(target, cv_predictions)
# Print the normal precision, recall, and F1 scores
print(f'Normal Precision: {precision}')
print(f'Normal Recall: {recall}')
print(f'Normal F1 Score: {f1}')
